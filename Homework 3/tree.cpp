...
int countIncludes(const string a1[], int n1, const string a2[], int n2)
{
    ...
    int nExtraCounter = countIncludes(a1+1, n1-1, a2, n2);

    if (a1[0] == a2[0])
        return nExtraCounter + countIncludes(a1+1, n1-1, a2+1, n2-1);
    else
        return nExtraCounter;
}
...
void order(string a[], int n)
{
  ...
  int nFirstGreater;
  int nFirstNotLess;
  split(a, n, a[0], nFirstNotLess, nFirstGreater);

  order(a, nFirstNotLess);
  order(a + nFirstGreater, n - nFirstGreater);
}